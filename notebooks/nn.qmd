...
Aside from the standard PyTorch and pandas imports, I also import three custom functions: <!-- `save_performance`, `train_model`, and `test_model`. The `save_performance` function saves the performance metrics of the model to a json file for future analysis. The `train_model` function trains the model based on the hyperparameters and data provided, prints the training and validation loss and accuracy in real-time, and saves the best model based on the iteration with the lowest validation loss. It also provides an option to visualize the training progress using the `PlotLosses` library or `matplotlib`. The `test_model` function evaluates the model on the test set using the best model saved during training and returns the testing accuracy. -->
